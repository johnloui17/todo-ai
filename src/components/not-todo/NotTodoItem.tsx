...
const NotTodoItem: React.FC<NotTodoItemProps> = ({ task, streak, onStatusChange }) => {
  ...
  // Extracted Tailwind class variables for readability
  const baseCardClasses = "flex flex-col gap-4 p-5 mb-4 rounded-[2rem] border transition-all duration-300";
  const statusCardClasses = isAvoided 
  ...
  const titleClasses = `text-lg font-bold transition-colors ${
    isAvoided ? 'text-emerald-900 dark:text-emerald-400' : 'text-red-900 dark:text-red-400'
  }`;

  const streakBadgeClasses = "flex items-center gap-1.5 px-3 py-1 rounded-full bg-orange-100 text-orange-600 dark:bg-orange-950/40 dark:text-orange-400 border border-orange-200 dark:border-orange-900/30 shadow-sm";

  const avoidedButtonClasses = `flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl font-bold text-sm shadow-sm transition-all active:scale-95 ${
    isAvoided 
  ...
  const failedButtonClasses = `flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl font-bold text-sm shadow-sm transition-all active:scale-95 ${
    isFailed 
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        ...
        <div className={streakBadgeClasses}>
      ...
      <div className="flex items-center gap-3">
        <button 
          ...
          className={avoidedButtonClasses}
        >
